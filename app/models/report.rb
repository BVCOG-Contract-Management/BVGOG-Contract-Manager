class Report < ApplicationRecord
    ...
    def generate_standard_users_report
      ...
      def generate_contract_expiration_report
        report = self
        print "Generating contract expiration report..."
        report.set_report_file
        ...
        contracts_31_60_days = Contract.where("ends_at >= ? AND ends_at <= ?", Date.today + 31.days, Date.today + 60.days).order(:ends_at)
        # Collect contracts expiring in the next 61-90 days
        contracts_61_90_days = Contract.where("ends_at >= ? AND ends_at <= ?", Date.today + 61.days, Date.today + 90.days).order(:ends_at)
